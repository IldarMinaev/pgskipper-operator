...
2. **OOMKilled**: Run `.apm/skills/_sql/memory_requirements.sql` to compare PG memory config against pod limits. Run `.apm/skills/_sql/memory_intensive_queries.sql` to find queries spilling to disk. See `postgresql-performance-check` for runaway queries. Increase pod memory limits if config exceeds limits.
